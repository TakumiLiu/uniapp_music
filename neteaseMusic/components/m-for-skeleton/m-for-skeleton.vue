<template>
	<view :class="[displayInfo,'skeleton']" :animation="animationData" v-if="show">
		<div class="skeleton-avatar" v-if="avatar" :style="{
		width:imgsize,
		height:imgsize,
		borderRadius:imgarc,
	}" />
		<view class="skeleton-content">
			...
			<view v-for="(item,key) in rowDataInfo" :key="key" class="skeleton-content-row"
				:style="{width:rowInfo(key)}" />
		</view>
	</view>
</template>
			...
			rowInfo(key) {
				...
				switch (rowTypeof) {
					...
					case 'object':
						...
						return (this.rowData[key].indexOf('%') > -1) ? this.rowData[key] :
							`${uni.upx2px(parseFloat(this.rowData[key]))}px`;
						break;
				}
			}
		}
