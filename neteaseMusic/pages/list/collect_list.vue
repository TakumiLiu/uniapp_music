	...
	export default {
		...
		methods: {
			getMusic() {
				if (!this.$u.utils.isLogin()) return
					...
					userInfo().then((res) => {
						...
						userCollectMusicId(this.useremail).then((res) => {
							for (let i = 0; i < res.data.length; i++) {
								// console.log(this.songList[i]);
								Promise.all([songDetail(res.data[i].music_id)]).then((values) => {
			...
			handleToDetail(songId) {
				...
				let item;
				let song_list = [];
				for (item in this.playlist) {
					song_list[item] = this.playlist[item][0].id;
				};
